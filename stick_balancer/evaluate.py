...
from recipes import env_kwargs_for


def load(run_dir: Path, algo: str, n_links: int, use_best: bool = True, phase: str = "balance"):
    env_kwargs = env_kwargs_for(n_links, phase)
...
def main() -> None:
    ...
    run_dir = args.run or Path("runs") / f"{args.algo}_{args.links}links{suffix}"
    model, venv = load(run_dir, args.algo, args.links, phase=args.phase)
    ...
    if args.export:
        ...
        args.export.write_text(json.dumps({
            "links": args.links, "algo": args.algo, "phase": args.phase, "return": total,
